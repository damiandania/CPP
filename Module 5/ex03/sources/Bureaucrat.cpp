...
/*   Updated: 2024/03/29 22:51:23 by damian           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(AForm & form) const {
	try {
		form.checkRequeriments(*this);
		// form.execute(*this);
		std::cout << YELLOW "[Bureaucrat] " << this->getName() << " executed "
			"[Form] " << form.getName() << RESET "\n";
	}
	catch (std::exception const & e) {
		std::cout << RED "[Bureaucrat] " << this->getName() << " couldn't execute "
			"[Form] " << form.getName() << " because " << e.what() <<RESET "\n";
	}
	return ;
}
